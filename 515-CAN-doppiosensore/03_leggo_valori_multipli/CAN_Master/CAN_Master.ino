...
void setup() {
  ...
  CAN.init_Filter(MCP_RXF0, 0, 0x10); //risponbde all'id=0x10 x T
  CAN.init_Filter(MCP_RXF1, 0, 0x11); //risponbde all'id=0x11 x X
...
void loop() {
  ...
  if (CAN_MSGAVAIL == CAN.checkReceive()){        
    ...
    Serial.print("tval=");Serial.println(remoteval);
    
      lcd.setCursor(0,1);
      lcd.print("t:");
      lcd.print(remoteval);  
    } else if (canid == 0x11) { //x
      //leggo il valore
    remoteval = (byte)buf[1];
    remoteval = remoteval << 8;
    remoteval |= (byte)buf[0];
    Serial.print("xval=");Serial.println(remoteval);
